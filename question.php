 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_checkpassword_question extends question_graded_by_strategy
    ...
    public function get_validation_error(array $response) {
        ...
        return get_string('pleaseenterananswer', 'qtype_checkpassword');
    }
    ...
    // Comparer la réponse utilisateur avec l'expression régulière définie dans $answer->answer.
    // $this->usecase est la sensibilité à la casse (case sensitivity).
    $pattern = $answer->answer; // Le motif regex stocké dans la base de données.

    // Utiliser 'i' pour ignorer la casse si nécessaire.
    $flags = $this->usecase ? '' : 'i';

    // Vérifier si la réponse utilisateur correspond à l'expression régulière.
    return preg_match('/' . $pattern . '/' . $flags, $response['answer']);
}
